...
def train(x, y, Ft, G, D, f_inp, f_out, opt_g, opt_d, device, S):
    ...
    (lx + 1e-3 * lg).backward(retain_graph=True)
    opt_g.step()
...
def main():
    ...
    # optimizers
    opt_g = optim.Adam(generator.parameters(), lr=1e-7)
    opt_d = optim.Adam(discrmntr.parameters(), lr=1e-7)
    ...
    f_inp.save(os.path.join(args.path, 'finetune/mask.png'))
    f_out = ApplyWatermark(watermark, wtmk_size=config['img_size'] // 2, norm=False, device=device)

    # log training progress
    log_file = open(os.path.join(args.path, 'finetune/history.csv'), 'w')
    log_file.write(','.join([
    ...
    def sample_image(epoch, model, root):
        ...
        sample_directory = os.path.join(args.path, f'finetune/{epoch}')
        ...
        for image in images:
            ...
            with torch.no_grad():
                ...
                lr_wm = f_inp(lr)
                sr_wm = clamp(model(lr_wm))
                hr_wm = f_out(hr.to(device))
                lr_wm, sr_wm, hr_wm = lr_wm.cpu(), sr_wm.cpu(), hr_wm.cpu()


                lr = F.interpolate(lr, size=size[::-1], mode='bicubic', align_corners=False).cpu()
                lr_wm = F.interpolate(lr_wm, size=size[::-1], mode='bicubic', align_corners=False).cpu()
                images = torch.cat([lr, sr, hr, lr_wm, sr_wm, hr_wm])
                save_image(images, os.path.join(sample_directory, os.path.basename(image)), nrow=2)
    ...
    while step < args.iter:
        ...
        try:
            ...
            if step % args.sample_freq < ngpu:
                ...
                save_model(generator, os.path.join(args.path, f'finetune/SRGAN.pt'))
                save_model(discrmntr, os.path.join(args.path, f'finetune/Dis96.pt'))
