...
def train(x, y, Ft, G, D, opt_g, opt_d, device, S):
    ...
    (lx + 1e-3 * lg + ls).backward(retain_graph=True)
    opt_g.step()
    ...
    return lx.item(), lg.item(), ldgx.item(), ldy.item(), ls.item()
...
def main():
    ...
    log_directory = create_log_directory(os.path.abspath('baseline_srgan/'))
    ...
    log_file.write(','.join([
        'loss_mse', 'loss_content', 'loss_adversarial', 'loss_dx', 'loss_dy', 'loss_s', 'time'
    ]) + '\n')
    log_file.flush()
    ...
    while not args.pretrained_SRResNet and step < args.pretrain_iteration:
        try:
            ...
            (loss_mse + loss_sign).backward(retain_graph=True)
            ...
            epoch += 1
            iter_loader = iter(loader)
            log_file.write(','.join([str(Loss_MSE.avg), *(['nan'] * 4), str(Loss_S.avg), str(time.time() - start)]) + '\n')
            log_file.flush()
            Loss_MSE.reset()
            Loss_S.reset()
            start = time.time()
    ...
    while step < args.iter:
        ...
        try:
            ...
            stats = train(lr_img, hr_img, feature, generator, discrmntr, opt_g, opt_d, device, args.sign_loss)
            ...
            if args.sign_loss: Loss_S.update(stats[4], n)
