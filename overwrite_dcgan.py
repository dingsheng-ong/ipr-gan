...
def train(epoch, loader, G, D, opt_G, opt_D, f_inp_new, f_inp_old, f_out, reg_coeff, device, log_directory):
    ...
    log_file = open(os.path.join(log_directory, 'overwrite/history.csv'), 'w')
    log_file.write(','.join([
    ...
    sample_img(Z, G, os.path.join(log_directory, f'overwrite/0.png'))
    ...
    for i in range(1, epoch + 1):
        ...
        loss_gz = - D(gz).mean()
        sloss_g = sign_loss(G)
        reg_loss = (1 - ssim(G(f_inp_new(z), update_stats=False), f_out(gz)))
        ...
        if i % 1000 == 0:
            ...
            log_file.flush()
            sample_img(Z, G, os.path.join(log_directory, f'overwrite/{i // 1000:03d}.png'))
            Loss_Gz.reset()
            Loss_Dx.reset()
            Loss_DGz.reset()
            SignLoss.reset()
            RegLoss.reset()
            save_model(G, os.path.join(log_directory, f'overwrite/generator.pt'))
            save_model(D, os.path.join(log_directory, f'overwrite/discrmntr.pt'))
...
def main():
    ...
    parser.add_argument('-lr', '--learning-rate', type=float, default=0.0001, metavar='LR',
                        help='learning rate for over-writing')
    ...
    f_inp_old = RandomBitMask(dim=128, nbit=10, const=-10, device=device)
    f_inp_old.load(os.path.join(args.path, 'watermark/mask.pt'))
    f_inp_new = RandomBitMask(dim=128, nbit=10, const=-10, device=device)

    # output transform function
    f_out = ApplyWatermark(args.watermark, wtmk_size=img_size // 2, norm=True, device=device)

    # create overwrite directory
    os.makedirs(os.path.join(args.path, 'overwrite'), exist_ok=True)

    # save transform functions meta data
    f_inp_new.save(os.path.join(args.path, 'overwrite/mask.pt'))
    f_out.save(os.path.join(args.path, 'overwrite/watermark.png'))

    train(args.epoch, loader, generator, discrmntr, opt_g, opt_d, f_inp_new, f_inp_old, f_out, reg_coeff, device, args.path)

    def save_model(model, path):
        torch.save(model.cpu().state_dict(), path)
        model.to(device)

    save_model(generator, os.path.join(args.path, f'overwrite/generator.pt'))
    save_model(discrmntr, os.path.join(args.path, f'overwrite/discrmntr.pt'))
